 ...
 * 
 */
package com.intuit.inventory.orderservice.exception;
...
import com.intuit.inventory.orderservice.controller.OrderController;
...
public class OrderExceptionAdvice {
	...
	private static final Logger LOGGER = LogManager.getLogger(OrderExceptionAdvice.class.getName());

	@ExceptionHandler({ ValidationException.class })
	public final ResponseEntity<ApiError> handleValidationException(ValidationException ex, WebRequest request) {
		HttpStatus status = HttpStatus.BAD_REQUEST;
		LOGGER.error("ValidationException occured for request :{}", request);
	...
	public ResponseEntity<ApiError> handleAllUncaughtException(Exception exception, WebRequest request) {
		ApiError apiError = new ApiError(HttpStatus.INTERNAL_SERVER_ERROR, "Unknown error occurred", exception,
				request);
		LOGGER.error("Exception occured for request :{}", request);
	...
	public ResponseEntity<ApiError> handleDataException(Exception ex, WebRequest request) {
		HttpStatus status = HttpStatus.BAD_REQUEST;
		LOGGER.error("DataIntegrityViolationException occured for request :{}", request);
		LOGGER.error("Exception :{}", ex.getLocalizedMessage());
		return handleExceptionInternal(new ApiError(status, ex.getLocalizedMessage(), ex));
	}
